...
{
	char *query =
	    "CREATE TABLE IF NOT EXISTS notes(id INTEGER PRIMARY KEY,title TEXT NOT NULL,text TEXT NOT NULL, date DATE NOT NULL)";
...
int insert_note(sqlite3 * handle, char *title, char *text)
{
	...
	sqlite3_stmt *stmt;
	char *query =
	    "INSERT INTO notes VALUES (NULL, ?, ?, datetime('now'))";
